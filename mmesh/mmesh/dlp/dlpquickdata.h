...
namespace mmesh
{
	...
	public:
		...
		void autoDlpSources(std::vector<DLPISource>& sources, AutoDLPSupportParam* autoParam, int flag= SUPPORT_VERTEX| SUPPORT_EDGE| SUPPORT_FACE, std::function<void(CallBackParams *)> callback=NULL, std::function<bool(CallBackParams*)> cb_interrupt = NULL, CallBackParams* cbParams=NULL);//7
