...
namespace mmesh 
{
	void getConvexHullMesh(trimesh::TriMesh* mesh, std::vector<trimesh::TriMesh*>& meshs)
	{
		...
		for (unsigned int polygon_id = 0; polygon_id < meshs.size(); ++polygon_id)
		{
			...
			if (discard)
			{
				meshs.erase(meshs.begin() + (polygon_id--));
				continue;
			}
			...
			//通过逆矩阵转换回三维坐标
			for (trimesh::point& apoint : meshs[polygon_id]->vertices)
			{
